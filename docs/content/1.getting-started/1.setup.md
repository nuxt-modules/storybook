...
Moreover, every pull request is automatically published to `pkg.pr.new`, allowing you to test the changes before they are merged into the main branch. The installation process is similar to the above, but now using the following in `package.json`:
