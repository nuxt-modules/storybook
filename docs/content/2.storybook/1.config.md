...
- with the `storybook:config` Nuxt hook
...
const defaultTheme = require('storybook/defaultTheme')
...
Learn more about the [Tailwind config](https://storybook.js.org /docs/configuration) in their docs.

### `config` option

You can also use your `nuxt.config` file to set your Tailwind config with the `storybook.config` property:

```js [nuxt.config]
import tailwindTypography from '@storybook/typography'
...
#### `storybook:resolvedConfig`

Passes the _complete_ resolved configuration with all defaults from [the full Tailwind config](https://github.com/tailwindlabs/storybook/blob/master/stubs/config.full.js) using [`resolveConfig`](https://github.com/tailwindlabs/storybook/blob/master/resolveConfig.js).
...
export default defineNuxtModule({
  ...
  setup (options, nuxt) {
    nuxt.hook('storybook:config', function (tailwindConfig) {
      tailwindConfig.plugins.push(tailwindTypography)
    })

    nuxt.hook('storybook:resolvedConfig', function (resolvedConfig) {
      // read: https://storybook.nuxtjs.org/tailwind/editor-support
      addTemplate({
